...
| `rembg_engine.py` | `remove_background(png: bytes) -> bytes`; lazy `new_session('u2net')` cache module-level; env `U2NET_HOME=server/data/models`                                                                                                                                                                                                                                                                                                                                               |
| `pose.py`         | `detect_pose(png: bytes) -> PoseResult {landmarks: dict[str, (x, y, visibility)], width, height}` — MediaPipe Tasks PoseLandmarker (model `pose_landmarker_lite.task`, tải về `server/data/models/` lần đầu); tọa độ pixel (không normalized)                                                                                                                                                                                                                               |
